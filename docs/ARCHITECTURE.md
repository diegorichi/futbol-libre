...
- `src/scraping/site_scraper.py` recorre sitios con un Chrome aislado por dominio y agrega `fuente` a eventos/opciones. Chrome usa carga `eager` para no esperar recursos secundarios; si aun así vence el timeout pero existe DOM, intenta extraerlo. Una sesión rota se cierra antes de continuar y su perfil temporal también se elimina.
- `src/scraping/event_extractor.py` combina estrategias HTML, PHP, canal directo, tiempo estructurado, agenda y menú; también recorre iframes. Si el DOM dinámico no llega a estabilizarse, intenta extraer el contenido disponible para permitir luego el fallback HTTP cuando siga vacío.
